...
static void normalize_type_name_for_wsh(char *type_name);
static void class_formatter_json(struct class *class, struct cu *cu, uint32_t id);
...
static void print_classes(struct cu *cu)
{
	...
	cu__for_each_struct_or_union(cu, id, pos) {
		...
		if (show_packable && !global_verbose)
			...
			if (json_format) {
				class_formatter_json(pos, cu, id);
			} else {
				formatter(pos, cu, id);
			}
		}
...
static void class_formatter_json(struct class *class, struct cu *cu, uint32_t id)
{
	...
	const char *class_name = class__name(class);

	// Handle anonymous structs
	if (class_name == NULL) {
		struct tag *typedef_alias = cu__find_first_typedef_of_type(cu, id);
		if (typedef_alias != NULL) {
			struct type *tdef = tag__type(typedef_alias);
			class_name = type__name(tdef);
		} else {
	...
	printf("  \"size\": %u,\n", class__size(class));
	printf("  \"alignment\": 8,\n");	// Default alignment
	printf("  \"description\": \"Generated by wpahole from DWARF debug info\",\n");
	printf("  \"fields\": [\n");
	...
	type__for_each_member(&class->type, pos) {
		...
		const char *member_name = class_member__name(pos);
		if (member_name == NULL) {
			member_name = "<anonymous>";
		}
		// Get member type name
		struct tag *member_tag = cu__type(cu, pos->tag.type);
		char type_name[256];
		if (member_tag) {
			tag__name(member_tag, cu, type_name, sizeof(type_name), NULL);
			// Normalize type name for wsh compatibility
			normalize_type_name_for_wsh(type_name);
		} else {
...
static void normalize_type_name_for_wsh(char *type_name)
{
	char *temp = strdup(type_name);
	if (!temp)
		return;

	// Remove const qualifier
	if (strncmp(temp, "const ", 6) == 0) {
		memmove(temp, temp + 6, strlen(temp + 6) + 1);
	}
	// Handle system types
	if (strcmp(temp, "__off_t") == 0 || strcmp(temp, "__off64_t") == 0) {
		strcpy(type_name, "long");
		free(temp);
		return;
	}
	if (strcmp(temp, "size_t") == 0) {
		strcpy(type_name, "unsigned long");
		free(temp);
		return;
	}
	// Simplify complex function pointers to void*
	if (strstr(temp, "()") != NULL) {
		strcpy(type_name, "void*");
		free(temp);
		return;
	}
	// Handle pointer types - remove spaces around *
	char *ptr = temp;
	char *dst = type_name;
	while (*ptr) {
		if (*ptr == ' ' && *(ptr + 1) == '*') {
	...
	*dst = '\0';
	...
	// Convert "struct name*" to "void*" (your system may not handle struct pointers)
	if (strncmp(type_name, "struct ", 7) == 0 && strstr(type_name, "*")) {
		strcpy(type_name, "void*");
	}
	// Handle typedefs that your system doesn't recognize
	if (strcmp(type_name, "FILE*") == 0) {
		strcpy(type_name, "void*");
	}
	if (strcmp(type_name, "_IO_lock_t*") == 0) {
		strcpy(type_name, "void*");
	}
	if (strcmp(type_name, "argp_parser_t") == 0) {
		strcpy(type_name, "void*");
	}
